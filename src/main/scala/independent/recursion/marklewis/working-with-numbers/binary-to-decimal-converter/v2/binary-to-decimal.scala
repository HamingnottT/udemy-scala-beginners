...
object BinToDecV2 {
    ...
    def getDecimal(binString: String): Int = {     // return type needs to be ': Int = '
        ...
        var binMultiples: ListBuffer[Int] = ListBuffer()
        var newListBuffer: ListBuffer[Int] = ListBuffer()
        var counter: Int = 0
        
        for (bit <- 0 to binListReversed.length) {
            binMultiples += pow(2, (bit + 1)).toInt
        }
 ...
 * 
 * UPDATE: program now supportive of more than 8-bit values
 *   This was achived by adding a new list buffer to contain the binary multiple values
 *   A for loop was added to append to the new buffer raising 2 to the power of n length of the reversed binary list
 * 
 ...
 *  - Or in a pinch subtract '0' by the char value or 48 ('0''s ASCII equivalent)
 * 
